...
bird = pygame.image.load("images/bird.jpeg").convert_alpha()
pipe = pygame.image.load("images/pipe.jpeg").convert_alpha()
rotatedPipe = pygame.image.load("images/rotated_pipe.jpeg").convert_alpha()
    ...
    def isCollide(self):
        for i in range(0, 7):
            if (self.birdX >= self.pipesX[i] and self.birdX <= (self.pipesX[i] + pipe.get_width())
                    and ((self, self.birdY + bird.get_height() - 15) >= self.lowerPipeY[i] or
